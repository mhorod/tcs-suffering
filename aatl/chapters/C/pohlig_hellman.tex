	...
	\begin{enumerate}
		...
		\item Jeśli \( n = p^k \), gdzie \( p \) jest pierwsze oraz \( k > 1 \), to:
		      \begin{enumerate}
			      ...
			      \item Na podstawie równania:
			            \[
				            b = g^x \implies b^{p^{k-1}} = \pars{g^x}^{p^{k-1}} = \pars{g^{x_0}}^{p^{k-1}},
			            \]
		...
		\item Jeśli \( n  = q_1 \cdot q_2 \), gdzie \( \gcd(q_1, q_2) = 1 \), to:
		      \begin{enumerate}
			      ...
			      \item Oblicz \( r_1 \) takie, że \(x = r_1 \pmod{q_1}\) rekurencyjnie rozwiązując równanie \( h^{r_1} = b^{q_2} \) na grupie \( H = \langle h \rangle \) rzędu \(q_1\).
			      \item Analogicznie, oblicz \( r_2 \) takie, że \( x = r_2 \pmod{q_2} \).
			      \item Z CRT wyznacz \( x \), rozwiązanie układu kongruencji \( x = r_i \pmod{q_i} \).
		      \end{enumerate}
		      Jeżeli \( n = p_1^{\alpha_1} \cdot \ldots \cdot p_s^{\alpha^s} \), to rekurencyjnie wywołamy przypadek 2 dla każdego \( p_i^{\alpha_i} \) \( \rightarrow \bigO(\sum_i \alpha_i \sqrt{p_i}) \leq \bigO(\log n \cdot \sqrt{\max{p_i}}) \), bo \( \alpha_i \leq \log n \).
	\end{enumerate}
\end{greyframe}
