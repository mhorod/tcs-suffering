...
\begin{proof}
	...
	\[
		a^m = \pars{g^{2k+1}}^m = \pars{g^k}^{2m} \cdot g^m = g^m = \xi = -1,
	\]
...
\]
Niezmiennik drugi oczywiście jest zachowany w 3. (a), a w 3. (b) jeśli ustawimy \(t = t + m\) to również jest zachowany bo \(m\) jest wielokrotnością \(2q\).
