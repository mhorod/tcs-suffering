\begin{theorem}[Chińskie Twierdzenie o Resztach]
	Dla parami względnie pierwszych liczb całkowitych \( m_1, \dots m_k \geq 1 \), \( M = m_1 \cdot \ldots \cdot m_k \) oraz liczb \( a_1, \ldots, a_k \), gdzie \( 0 \leq a_i < m_i \), układ kongruencji:
	\begin{center}
		\( x = a_1 \pmod{m_1} \) \\
		\( \dots \) \\
		\( x = a_k \pmod{m_k} \)
	\end{center}
	ma dokładnie jedno rozwiązanie \( 0 \leq x < M \).
\end{theorem}
\begin{proof}
	Trzeba udowodnić, że rozwiązanie istnieje i że wszystkie rozwiązania przystają do siebie modulo \( M \). \\
	\textit{Istnienie}\footnote{Dowód został zaczerpnięty z \href{https://en.wikipedia.org/wiki/Chinese\_remainder\_theorem\#Proof}{,,Chinese remainder theorem'', Wikipedia}.} \\
