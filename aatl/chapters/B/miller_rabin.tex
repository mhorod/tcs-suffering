Każda liczba pierwsza \( p \) spełnia Małe Twierdzenie Fermata: \[ a^{p-1} = 1 \pmod{p} \] Byłby to dobry test pierwszości, gdyby nie to, że specyficzne liczby złożone \(p\) też mogą go zdać (dla wszystkich \( a \)). Algorytm Millera-Rabina rozwiązuje, a przynajmniej minimalizuje ten problem.
...
\begin{greyframe}
	...
	\begin{enumerate}
		...
		\item Oblicz \( (r_1, \dots, r_{s+1} ) = (a^k, a^{2k}, \dots, a^{2^{s-1}k}, a^{n-1}) \).
		\item Jeśli \( (r_1, \dots, r_{s+1}) = (1, \dots, 1) \), zwróć PIERWSZA.
		\item Jeśli \( (r_1, \dots, r_{s+1}) = (\dots, -1, 1, \dots, 1) \), zwróć PIERWSZA.
		\item Zwróć ZŁOŻONA.
	\end{enumerate}
\end{greyframe}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item \( n = p^k \) jest potęgą liczby pierwszej. \\
		      ...
		      Gdyby było tak, że \( \beta^{n-1} = 1 \pmod{n} \), to również \( \beta^{n-1} = 1 \pmod{p^2} \), czyli \( \beta^n = \beta = p + 1 \pmod{p^2} \), co daje sprzeczność. Zatem \( \beta^{n-1} \neq 1 \pmod{n} \).
