...
\begin{definition}
	Dla \( i \in S \) definiujemy \(\mathcal{T}(i) = \set{t \geq 1 \mid p_{ii}(t) > 0}\). Jest to zbiór takich \( t \), że jesteśmy w stanie dojść z \( i \) do \( i \) w t krokach.
\end{definition}
\begin{definition} 
	\textbf{Okres stanu} dla \(i \in S\) definiujemy jako \(o(i) = \gcd\pars{\mathcal{T}(i)}\)
\end{definition}
...
\begin{proof}
	...
	\]
	Mamy teraz \(o(i) \mid n\), a wiec \(o(i)\) jest dzielnikiem każdego elementu \(\mathcal{T}(j)\), a więc jest \(\leq\) od jego nwd, a więc \(o(i) \leq o(j)\). Analogicznie dowodzimy w drugą stronę, otrzymując równość.
\end{proof}

\begin{lemma}
	Jeśli \( \set{ X_t \mid t \in \natural } \) jest nieokresowym i nieprzywiedlnym łańcuchem Markowa, to:
	\[
...
\begin{definition}
	...
	a \textbf{chwilowy} jeśli \( f_{i, i} < 1 \). Mówimy, że łańcuch jest powracający jeśli każdy jego stan jest powracający.
\end{definition}
...
\begin{definition}
	Stan jest \textbf{ergodyczny} jeśli jest nieokresowy i dodatnio powracający. Łańcuch jest ergodyczny jeśli każdy jego stan jest ergodyczny.
\end{definition}

\begin{lemma}
	W skończonym procesie Markowa:
	\begin{enumerate}
		\item istnieje co najmniej jeden stan powracający
		\item każdy stan powracający jest dodatnio powracający
	\end{enumerate}
...
\begin{lemma}
	Skończony, nieprzywiedlny, nieokresowy łańcuch Markowa jest ergodyczny
\end{lemma}

\begin{lemma}
	Jeśli stan \(x\) jest powracający to
	wszystkie stany w tej samej klasie skomunikowania również są powracające.
\end{lemma}
\begin{proof}
	Wybierzmy dowolny stan \( y \) w tej samej klasie skomunikowania co \( x \).
	Pokażemy, że jest on powracający.
	...
	\begin{enumerate}
		...
		\item Wychodząc ze stanu \( x \) zanim wrócimy do \( x \) to napotykamy \( y \) \\
		      Ponieważ \( x \) jest powracający to to zdarzenie ma szansę \( 1 - p \)
	...
	nigdy już do niego nie wrócimy, co jest sprzeczne z założeniem że jest on powracający.
	...
	Aby pokazać, że \( y \) jest powracający pokażemy, że prawdopodobieństwo na to, że wychodząc z \(y \) od pewnego momentu nigdy już nie wrócimy do \( y \) jest zerowe.
