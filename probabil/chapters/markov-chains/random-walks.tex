\begin{definition}
	\textbf{Spacerem losowym} na nieskierowanym grafie \( G \) nazywamy łańcuch Markowa, którego stany odpowiadają wierzchołkom grafu. Prawdopodobieństwo przejścia ze stanu \( v \) do stanu \( u \) wynosi \( \frac{1}{\text{deg}(v)} \) gdy \( (v, u) \in E\) i 0 w przeciwnym przypadku.
\end{definition}
...
\begin{proof}
	...
	\[ \sum_{v\in V \left( G  \right) }^{} \pi_v = \sum_{v\in V \left( G  \right) }^{} \frac{d \left( v  \right) }{2 |E| } = \frac{1}{2 |E|} \sum_{v\in V \left( G  \right) }^{} d \left( v  \right) = 1, \]
	a więc faktycznie jest to rozkład. Mamy też
	\[ \left( \pi P \right)_v  = \sum_{u \in V \left( G  \right)}^{} \pi_u \cdot P_{uv} = \sum_{u \in N \left( v \right) }^{} \frac{d \left( u  \right) }{2 |E| } \cdot \frac{1}{d \left( u \right) } = \frac{d \left( v  \right) }{2 |E| } = \pi_v,\]
...
\begin{definition}
	Zmienna losowa \(r_{uv}\) określa liczbę kroków spaceru losowego przebiegającego od wierzchołka \(u\) do wierzchołka \(v\).
\end{definition}

\begin{lemma}
	Dla każdej krawędzi \(uv\) w grafie \(G\) zachodzi \(\mathbb{E}\left[ r_{uv} \right] + \mathbb{E}\left[ r_{vu} \right] \le 2 |E| \).
\end{lemma}
\begin{proof}
	...
	W takim łańcuchu rozkład jednostajny \(\pi_{uv} = \frac{1}{2 |E_G| }\) jest stacjonarny. Po pierwsze \( \sum_{uv \in E_D }^{} \pi_{uv} = \sum_{uv \in E_D }^{} \frac{1}{2 |E_G| } = 1\), więc jest to rozkład.
	Mamy też \[ \sum_{w\in N\left( u  \right) }^{} \pi_{wu} \frac{1}{d\left( u \right) } = \frac{1}{d\left( u  \right) }\cdot \frac{d\left( u  \right) }{2 |E_G| } = \frac{1}{2|E_G| } = \pi_{uv},\]
	...
	\[ \mathbb{E}\left[ r_{uv} \right] + \mathbb{E}\left[ r_{vu} \right] \le \mathbb{E}\left[ r_{(vu)(vu)} \right] = \frac{1}{\pi_{vu}} = 2|E_G| \]
\end{proof}

\begin{theorem}[Twierdzenie 7.15 P\&C] Wartość oczekiwana czasu pokrycia grafu \( G = (V, E) \) jest ograniczona od góry przez \( 2|E|\left( |V|-1 \right)  \).
\end{theorem}
\begin{proof}
	...
	\[ \mathbb{E}\left[ C_G \right] \le \sum_{i=0}^{2\left|V\right|-3} \mathbb{E}\left[ r_{v_iv_{i+1}} \right] = \sum_{xy\in T}^{} \mathbb{E}\left[ r_{xy} \right] + \mathbb{E}\left[ r_{yx} \right] \le 2|E|\cdot \left( \left|V\right|-1 \right)  .\]
\end{proof}
...
\begin{proof} Zauważmy, że spacer po naszym grafie \(G\) ma rozkład stacjonarny. Dlaczego? Możemy odhaczyć wszystkie wymagania:
	...
	\end{itemize}
	\begin{enumerate}[label=(\roman*)]
		\item Wiemy, że z~własności rozkładu w~skończonym, nieprzywiedlnym i~ergodycznym łańcuchu Markowa
		      \begin{equation*}
		...
		\item Ułatwimy sobie trochę życie i~policzymy oczekiwaną liczbę odwiedzeń \(B\)~\emph{lub} \(C\) przed pierwszym powrotem do \(A\). Odpowiedź do zadania będzie równa połowie z~tego wyniku, ponieważ sytuacja jest symetryczna.
		      ...
		      Ponieważ graf jest skończony i~spójny, to wszystkie stany są powracające. Możemy więc powiedzieć, że mamy rozkład geometryczny z~parametrem \(\frac{1}{4}\): za każdym razem, gdy jesteśmy w~\(B\) lub \(C\) mamy prawdopodobieństwo sukcesu --- powrotu do \(A\) --- równe \(\frac{1}{4}\), a~porażka, ze względu na to że stan jest powracający, ostatecznie zaprowadzi nas z~powrotem do stanu ,,\(B\) lub \(C\)'' (być może długą drogą, ale w~łańcuchu Markowa trasa, którą obraliśmy, nie ma znaczenia) i~postawi przed takim samym eksperymentem. Zatem tak naprawdę oczekujemy po prostu na sukces, który wypada z~szansą \(\frac{1}{4}\). Oczekiwana liczba prób wynosi zatem \(4\), a~odpowiedź do naszego zadania \(2\).
	\end{enumerate}
\end{proof}
