...
\begin{proof}
    \begin{align*}
        M_X(t) &= \int_{a}^{b} e^{tx} \cdot \frac{1}{b-a} \, dx \\
...
\begin{proof}
	\begin{align*}
		P(X \leq c \mid X \leq d) = \frac{P\left( X\le c \cap X\le d  \right) }{P\left( X\le d  \right) } = \frac{P\left( X\le c  \right) }{P\left( X\le d  \right) } = \frac{c-a}{b-a} \cdot \frac{b-a}{d-a} = \frac{c-a}{d-a}.
	\end{align*}
\end{proof}
