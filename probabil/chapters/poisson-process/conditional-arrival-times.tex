...
\begin{theorem}[Twierdzenie 8.14 P\&C]
	Niech \(\left\{ N\left( t  \right) \mid t\ge 0 \right\} \) będzie procesem Poissona z parametrem \(\lambda\). Niech \(T_i\) będzie czasem przyjścia \(i\)-tego zdarzenia. Przy warunku \(N\left( t  \right) = n \) rozkład \(\left( T_1,\ldots,T_n \right) \) jest taki sam jak \(\mathrm{sort}\left( X_1,\ldots,X_n \right) \), gdzie zmienne \(X_1,\ldots,X_n\) mają rozkład jednostajny na \([0,t]\) i są niezależne.
\end{theorem}
\begin{proof}
	...
	\end{align*}
	gdzie trzecie przejście jest policzeniem najbardziej wewnętrznej całki (wychodzi \(\lambda^n e^{-\lambda t}\) co jest stałą względem pozostałych całek, więc wyciągamy to na przód), a później podstawiamy \(u_i = \sum_{j=1}^{i} z_j\) (całkujemy funkcję stałą, więc znaczenie ma tak naprawdę tylko długość przedziału).

	Wiemy, że \(P\left( N\left( t  \right) = n  \right) = \frac{e^{-\lambda t }\left( \lambda t  \right) ^{n}}{n!}\), więc prawdopodobieństwo warunkowe będzie wynosiło:
	\begin{align*}
		\prob\pars{\pars{T_1, \ldots, T_n} \leq \pars{s_1, \ldots, s_n} \mid N\pars{t} = n} & = \frac{\lambda^n e^{-\lambda t} \int_{u_1 = 0}^{s_1} \ldots \int_{u_n = u_{n-1}}^{s_n} \diff u_n \ldots \diff u_1}{\frac{e^{-\lambda t}\pars{\lambda t}^n}{n!}} \\
		& = \frac{n!}{t^n} \int_{u_1 = 0}^{s_1} \ldots \int_{u_n = u_{n-1}}^{s_n} \diff u_n \ldots \diff u_1
	\end{align*}
\end{proof}
