...
\begin{theorem}[Twierdzenia 8.9 i 8.10 P\&C]
...
\begin{proof} Pierwsze pokażmy, że jest to prawda dla \(X_1\)
	...
	\begin{align*}
		\prob\pars{X_i > t_i \mid (X_0, X_1, \dots, X_{i-1}) = (t_0, t_1, \dots, t_{i-1})} & = \prob\pars{N\pars{\sum_1^i t_j} - N\pars{\sum_1^{i-1} t_j} = 0} \\
		                                                                                   & = \prob\pars{N(t_i) = 0} = e^{-\lambda t_i}
	\end{align*}
	A więc \(X_i\) też ma rozkład wykładniczy z parametrem \(\lambda\), i jest niezależne od wartości poprzednich międzyczasów.
\end{proof}

\begin{theorem}[Twierdzenie 8.11 P\&C]
	Niech \(\set{N(t) \mid t \geq 0}\) będzie stochastycznym procesem liczącym takim, że
	\begin{enumerate}
		\item N(0) = 0
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Niech \( X_1 \) będzie zmienną opisującą czas do pierwszego zdarzenia, a \( X_2 \) od pierwszego zdarzenia do drugiego. Widzimy, że \( P(N(t) = 1) = P(X_1 < t \land X_1 + X_2 > t) \)
		      ...
		      Skorzystamy zatem z twierdzenia o trzech funkcjach aby udowodnić zadaną granicę.
		...
		\item Ostatni warunek szacujemy niemal identycznie jak poprzedni. \\
		      ...
		      \begin{align*}
			      \lim_{t \rightarrow 0} \frac{\prob(X_1 < t \land X_2 < t)}{t}
			       & = \lim_{t \rightarrow 0} \frac{\prob(X_1 < t)\prob(X_2 < t)}{t}                                       \\
			       & = \lim_{t \rightarrow 0} \frac{\pars{1 - \exp(-\lambda t)}\pars{1 - \exp(-\lambda t)}}{t}             \\
			       & = \lim_{t \rightarrow 0} \frac{1 + \exp(-2\lambda t) - 2\exp(-\lambda t)}{t} = \brackets{\frac{0}{0}} \\
			       & = \lim_{t \rightarrow 0} -2\lambda\exp(-2\lambda t) + 2\lambda\exp(-\lambda t)                        \\
			       & = -2\lambda + 2\lambda = 0
		      \end{align*}

		      Wyszła nam granica jaką chcieliśmy, a więc z twierdzenia o trzech funkcjach
		      \[
