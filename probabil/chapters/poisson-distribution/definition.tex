%<*probabil-egzamin-6-wlasnosci-rozkladu-poissona-1>
%<*probabil-2025-11-07-poisson-1>
\begin{definition}
	Mówimy, że zmienna losowa \( X \) ma rozkład
	\textbf{Poissona} z parametrem \( \lambda \) jeśli
	\[
		\forall_{n \in \natural} \, \prob(X = n) = e^{-\lambda} \cdot \frac{\lambda^n}{n!}
	\]
\end{definition}
...
\begin{proof}
%</probabil-egzamin-6-wlasnosci-rozkladu-poissona-1>
	Są dwie ścieżki aby to pokazać -- pierwszy polega na przeliczeniu explicite \( P(X + Y = n) \).

	\begin{align*}
		\prob\pars{X+Y=n} &= \sum_{k=0}^{\infty}\prob\pars{X = k \cap Y = n-k}												\\
		&= \sum_{k=0}^{\infty}\prob\pars{X = k}\prob\pars{Y = n-k}															\\
		&= \sum_{k=0}^{\infty}\frac{e^{-\lambda_X}\lambda_X^k}{k!}\cdot\frac{e^{-\lambda_Y}\lambda_Y^{n-k}}{\pars{n-k}!}	\\
		&= \frac{e^{-\lambda_X-\lambda_Y}}{n!}\sum_{k=0}^{\infty}\frac{\lambda_X^k\lambda_Y^{n-k}n!}{k!\pars{n-k}!}			\\
		&= \frac{e^{-\lambda_X-\lambda_Y}}{n!}\sum_{k=0}^{\infty}\lambda_X^k\lambda_Y^{n-k}\binom{n}{k}						\\
		&= \frac{e^{-\lambda_X-\lambda_Y}}{n!}\pars{\lambda_X+\lambda_Y}^n
	\end{align*}

	Co daje nam rozkład Poissona z parametrem \(\lambda_X + \lambda_Y\).
\end{proof}
\begin{proof}
	Pokażemy teraz bardziej elegancki dowód korzystający z funkcji tworzących momentów.
%<*probabil-egzamin-6-wlasnosci-rozkladu-poissona-2>
