...
\begin{definition}
	...
	\begin{itemize}
		\item \(\mu(\emptyset) = 0\)
		\item \(E_1, E_2, \dots \in \Sigma \text{ parami rozłączne} \implies \mu(\bigcup_{i=1}^\infty E_i) = \sum_{i=1}^{\infty} \mu(E_i)\)
	\end{itemize}
\end{definition}
...
\begin{definition}
	Funkcja \(f: \Omega_1 \to \Omega_2\) jest \textbf{mierzalna} dla przestrzeni mierzalnych \((\Omega_1, \Sigma_1), (\Omega_2, \Sigma_2)\) gdy
	\[
...
\begin{lemma}
	...
	\[
		\mu\pars{\bigcup_{i=1}^\infty A_i} = \lim_{i \to \infty} \mu(A_i)
	\]
\end{lemma}
...
\begin{lemma}
	...
	\[
		\mu\pars{\bigcap_{i=1}^\infty A_i} = \lim_{i \to \infty} \mu(A_i)
	\]
\end{lemma}
% TODO: dowód (nie był podany na wykładzie)

\begin{example}
	\textbf{Miara licząca} dla \(|\Omega| < \infty\) to \(\mu(A) = |A|\)
\end{example}

\begin{example}
	\textbf{Miara Lebesgue'a} to uogólnienie pojęcia długości, pola powierzchni i objętości na większą liczbę wymiarów. Przykładowo, w jednym wymiarze:
	\[
		\mu([a, b]) = b - a
	\]
	a więc
